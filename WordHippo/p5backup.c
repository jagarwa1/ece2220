...
// ECE 2220 Prog 5
// Purpose          
// Assumptions      - the user must input the advanced search fields in sequential order
// Known Bugs       
...
// remove an element from a dynamically allocated 2d array
// takes a reference to a 2d pointer to pointer array, the total size, and the index to remove
// returns nothing
        ...
        while (1){
            switch(input){
                case 1: // word length
                    ...
                    midArr = wordLength(midArr, outputArr, &count, &totalNums, midTotalNums, lengthOfWord);
                    break;
                case 2: // exclude 
                    printf("Excluding: ");
                    scanf("%s", containLett);
